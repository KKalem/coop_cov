...
class Agent(object):
    ...
    def __init__(self,
        ...
        self.current_dubins_points = None


    @property
    def color(self):
        return Agent.COLORS[self.id%len(Agent.COLORS)]
    ...
    def update(self,
        ...
        if current_timed_wp is None:
            ...
            dist = geom.euclid_distance(self.internal_auv.pose[:2], current_timed_wp.pose[:2])
            # at_target = self.internal_auv.reached_target or (dist < self.internal_auv.target_threshold)
        ...
        # if dist > self.mission_plan.config['turning_rad']+0.5:
        if dist > self.internal_auv.target_threshold + 0.5:
            # plan a dubins path from current estiamte location to targe pose
            # and give a point on that path to the controller to follow
            ...
            # and then use the closest point that is far away enough that the auv 
            # would consider it not reached
            for pt in pts:
                dist = geom.euclid_distance(self.internal_auv.pose[:2], pt[:2])
                if dist > self.internal_auv.target_threshold + 0.1: # with some margin
                    target_posi = pt[:2]
                    break

        if target_posi is None:
            # there is no point?
            # then just take the last point in the path... we do our best i guess
...
if __name__ == '__main__':
    ...
    mplan = MissionPlan(plan_type = MissionPlan.PLAN_TYPE_DUBINS,
                        num_agents = 1,
                        swath = 50,
                        rect_width = 100,
                        rect_height = 100,
                        ...
                        kept_uncertainty_ratio_after_loop = 0.2,
                        turning_rad = 5)
    ...
    with tqdm(total = mplan.last_planned_time) as pbar:
        while True:
            step += 1
            for agent in agents:
                _, _, drift_angle = drift_model.sample(agent._real_auv.pos[0],
                                                       agent._real_auv.pos[1])

                agent.update(dt = dt,
                             drift_trans_angle = drift_angle,
                             drift_trans_k = mplan.config['uncertainty_accumulation_rate_k'])
    ...
    for agent in agents:
        agent.visualize(ax)
        ax.plot(p[:,0], p[:,1], c=agent.color, marker='x')
